...
import { deleteRequest, deleteRequestTable, getRequest, postTable } from 'src/tools/request'
...
interface Props {
  ...
  openDelete: boolean
  row: ITable | undefined
  fetchData: ()=> void
}
export default function DeleteTable({  handleCloseDeletePosition,
  openDelete,
  row,
  fetchData,}: Props) {
  const classes = useStyles()

  const tableDeleteRequest = async (event: { preventDefault: () => void }) => {
    event.preventDefault()
    try {
      await postTable({
        url: `restaurants/admin/table/${row?.id}/delete/`,
        body: {
              ...
              <Grid item style={{ paddingBottom: '16px' }}>
                <ThemeProvider theme={fonttheme}>
                  <Typography variant='body2' align='center'>
                    Вы действительно хотите удалить столик?
                  </Typography>
                </ThemeProvider>
              </Grid>
